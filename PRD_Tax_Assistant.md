...
Build a small agentic system that helps a person file a U.S. federal income tax return (Form 1040) by chatting with them. A user shows up with a single W-2 from a job paying roughly $40,000/year, has a short, friendly conversation, and walks away with a completed 2025 Form 1040 they can download.

You are given two things: the four pillars your harness must demonstrate, and the end result it must produce. Everything in between is your decision. Part of the challenge — and part of how you'll be judged — is the architecture you choose to get from the pillars to the result.

## The Four Pillars (required)

Your agent must be built on a harness that clearly demonstrates all four. How you implement, enforce, and expose each one is up to you — but a judge should be able to point at your code and your running system and see each pillar working.

- Chat loop — a conversational loop between the user and the agent that carries state across turns.
- Tools — the agent can take real actions through defined tools, not just talk. At minimum, something that produces the filled return.
- Guardrails — constraints that keep the agent on-task, safe, and bounded (what it will and won't do, validation of what it accepts, limits it respects).
- Observation — the agent's behavior is observable: you (and a judge) can see what it did, the decisions it made, and the actions it took.
...
This is the design space. Pick what gets you to the end result best, and be ready to explain why:
...
If you find yourself blocked on a "but the spec doesn't say X" — that's the point. Make a reasonable call, document it, move on.
...
Not judged: visual/UI polish. Keep the front end minimal. Spend your effort on the harness and on making it work.
...
You have the pillars and the destination. The route is yours to design. Make deliberate choices, keep it working, and keep it simple.